...
{
    lista *newNode;
    newNode = (lista *)malloc(sizeof(lista)); // inicializar apontador de alocacao dinamica
...
void imprimePonterio(lista *head) /// para imprimir resultado da função busca
{
    if (head == NULL) // Caso a função busca restorne null
    {
        printf("Não achou!!");
        return;
    }
